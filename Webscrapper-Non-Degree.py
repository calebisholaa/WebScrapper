...
for program in programsResult.find_all('details'):
    ...
    if program.find(string="Contact Information") != None:
        contactInfo = program.find(string="Contact Information").findNext('ul').get_text()


        phoneIndex = re.search(r'Phone:', contactInfo)
        emailIndex = re.search(r'Email', contactInfo)

        if phoneIndex != None:
            if emailIndex != None:
                contactName = contactInfo[0 : phoneIndex.start()].strip()
                ...
                contactName = contactInfo[0 : emailIndex.start()].strip()
                contactPhone = "Null"
                contactEmail = contactInfo[emailIndex.start() + 7:].strip()
